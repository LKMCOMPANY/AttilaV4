 ...
 * (`src/app/actions/device-proxy.ts`) and the native REST routes
 * (`/api/devices/[id]/proxy/…`). See each core for the operational notes.
 */
...
export async function verifyDeviceProxyCore(
  ctx: RequestSession,
  deviceId: string,
): Promise<VerifyProxyResult> {
  try {
    const { deviceId: id, dbId, tunnelHostname } = await resolveDeviceAccess(ctx, deviceId);
...
export async function updateDeviceProxyCore(
  ...
  try {
    ...
    // The real password is never sent to the client (redacted on read), so an
...
export async function clearDeviceProxyCore(
  ctx: RequestSession,
  deviceId: string,
): Promise<UpdateProxyResult> {
  ...
  try {
    const { deviceId: id, dbId, tunnelHostname } = await resolveDeviceAccess(ctx, parsed.data);
