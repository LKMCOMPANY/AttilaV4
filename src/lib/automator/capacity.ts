...
/**
 * Capacity estimation core — the single implementation behind the
 * Server Action (`src/app/actions/capacity.ts`) and the native REST
 * route (`/api/capacity/estimate`). Queries Gorgone server-side and
 * runs the REAL pipeline filter on a recent sample, so the estimate
 * can never drift from runtime behaviour.
 */
...
export async function getCapacityEstimateCore(
  ctx: RequestSession,
